...
#define DEFAULT_TITLE_MESSAGE "Altere as tiles usando Q e E"
...
static constexpr int OBJECT_TEXTURE_COUNT = 1;
...
{
	const char *objectFiles[OBJECT_TEXTURE_COUNT] = {"assets/Decor_TrashBag.png"};
...
int main()
{
	...
	while (!glfwWindowShouldClose(g_window))
	{
		...
		// Tile cycling with Q (prev) and E (next)
		if ((current_seconds - lastMoveTime) >= moveCooldown)
		{
			int col = sceneObjects[0].col;
			int row = sceneObjects[0].row;
			int tilesCount = (tileSetCols * tileSetRows) - 1;
			int curId = (int)tmap->getTile(col, row);

			if (GLFW_PRESS == glfwGetKey(g_window, GLFW_KEY_E))
			{
				int nextId = (curId + 1) % tilesCount;
				tmap->setTile(col, row, nextId);
				lastMoveTime = current_seconds;
				cout << "Current tile id: " << curId << " - Next tile id: " << nextId << endl;
			}
			else if (GLFW_PRESS == glfwGetKey(g_window, GLFW_KEY_Q))
			{
				int prevId = (curId - 1 + tilesCount) % tilesCount;
				tmap->setTile(col, row, prevId);
				lastMoveTime = current_seconds;
				cout << "Current tile id: " << curId << " - Previous tile id: " << prevId << endl;
			}
		}
